...
class BanAnFragment : Fragment() {
    ...
    var resultLauncherEdit = registerForActivityResult(
        ...
        if (result.resultCode == Activity.RESULT_OK) {
            ...
            if (ktra) {
                HienThiDSBan()
                Toast.makeText(activity, resources.getString(R.string.edit_sucessful), Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(activity, resources.getString(R.string.edit_failed), Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun HienThiDSBan() {
        ...
        banAnAdapter = activity?.let { BanAnAdapter(it, R.layout.icon_ban_an, banAnList!!) }
